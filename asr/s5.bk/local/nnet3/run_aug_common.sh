...
n_utts_train=$(cat data/${train_set}/utt2dur|wc -l) 
# n_utts_diagubm=$(( $n_utts_train / 3 ))
n_utts_diagubm=30000
lda_mllt_ali=tri2_ali_${n_utts_train}_nodup
...
if [ $stage -le 0 ]; then
  ...
  if [ ! -f data/$train_set/reco2dur ]; then
    utils/data/get_reco2dur.sh --nj $njobs --cmd "$train_cmd" data/$train_set || exit 1;
  fi

  # Make a version with reverberated speech
  rvb_opts=()
  rvb_opts+=(--rir-set-parameters "0.5, $rirs_noise_dir/simulated_rirs/smallroom/rir_list")
  rvb_opts+=(--rir-set-parameters "0.5, $rirs_noise_dir/simulated_rirs/mediumroom/rir_list")
...
if [ $stage -le 2 ]; then
  ...
  steps/make_mfcc.sh --cmd "$train_cmd" --nj $njobs \
                     data/${train_set}_aug exp/mfcc/${train_set}_aug $mfccdir
  steps/compute_cmvn_stats.sh data/${train_set}_aug exp/mfcc/${train_set}_aug $mfccdir
...
if [ $stage -le 3 ] && $generate_alignments; then
  ...
  steps/copy_ali_dir.sh --nj $njobs --cmd "$train_cmd" \
...
if [ $stage -le 5 ]; then
  ...
  for dataset in $test_set $maybe_rt03; do
    ...
    # Create MFCCs for the  set
    utils/copy_data_dir.sh data/$dataset data/${dataset}_hires
    steps/make_mfcc.sh --cmd "$train_cmd" --nj $njobs --mfcc-config conf/mfcc_hires.conf \
...
if [ "$use_ivectors" == "true" ]; then
  if [ $stage -le 6 ]; then
    ...
    utils/subset_data_dir.sh data/${train_set}_aug_hires $n_utts_diagubm data/${train_set}_aug_${n_utts_diagubm}_hires
    utils/data/remove_dup_utts.sh 200 data/${train_set}_aug_${n_utts_diagubm}_hires data/${train_set}_aug_${n_utts_diagubm}_nodup_hires  # 33hr
    ...
    utils/subset_data_dir.sh data/${ivector_trainset}_aug_hires $n_utts_train data/${ivector_trainset}_aug_hires_subset
  ...
  if [ $stage -le 8 ]; then
    ...
    steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 --num-frames 200000 \
      data/${train_set}_aug_${n_utts_diagubm}_nodup_hires 512 exp/nnet3/tri3b exp/nnet3/diag_ubm
  fi
